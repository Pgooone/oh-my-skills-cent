...
- [ ] **AC5 只读模式**：▲`OMS_BIND=0.0.0.0` 无 readonly → 拒启动；readonly 下写端点全 403（verifier 逐端点负向脚本）、浏览/导出 200、真实 TCP 验证 ConnectInfo 限流触发；访客上传 → ▲测试仓出现分支/PR ●只读 UI（写按钮不渲染/横幅）+ **pageErrors=0**
